#!/usr/bin/env python3
"""AI Chat E2E test: login, send queries in different modes, screenshot results."""
import asyncio
import json
import os
import sys
from pathlib import Path

from playwright.async_api import async_playwright

BASE_URL = "http://localhost:5173"
SCREENSHOT_DIR = Path(__file__).parent
QUERIES = [
    ("帮我研究下比亚迪的市场趋势", "byd-research"),
    ("分析家庭资产负债，联网搜索优化建议", "asset-analysis"),
]
MODES = ["普通", "Pro", "Ultra"]  # DeerFlow mode names in UI


async def screenshot(page, name):
    path = SCREENSHOT_DIR / f"{name}.png"
    await page.screenshot(path=str(path), full_page=False)
    print(f"  ✓ screenshot: {name}.png")


async def wait_for_stream_done(page, timeout_ms=180000):
    """Wait until streaming indicator disappears (response complete)."""
    start = asyncio.get_event_loop().time()
    while (asyncio.get_event_loop().time() - start) * 1000 < timeout_ms:
        streaming = await page.evaluate("""() => {
            // Check for any active streaming/typing indicators
            const indicators = document.querySelectorAll(
                '[class*="streaming"], [class*="typing"], [class*="Thinking"], .three-dot-loader'
            );
            return indicators.length > 0;
        }""")
        if not streaming:
            await asyncio.sleep(2)
            return True
        await asyncio.sleep(1)
    print("  ⚠ stream timeout")
    return False


async def extract_metrics(page):
    return await page.evaluate("""() => {
        const text = document.body.innerText;
        // Token usage
        const tokenEl = document.querySelector('[class*="token-usage"], [class*="TokenUsage"]');
        const tokenText = tokenEl ? tokenEl.innerText : '';
        // Chain of thought
        const cotEl = document.querySelector('[class*="chain-of-thought"], [class*="ChainOfThought"]');
        const hasCoT = cotEl !== null;
        // Citations
        const citations = document.querySelectorAll('[class*="citation"], [class*="Citation"]');
        // Tool calls
        const toolCalls = document.querySelectorAll('[class*="tool-call"], [class*="ToolCall"]');
        // Messages
        const messages = document.querySelectorAll('[class*="message-group"], [class*="MessageGroup"]');
        // Mode selector text
        const modeEl = document.querySelector('[class*="mode-selector"], [class*="ModeSelector"]');
        const modeText = modeEl ? modeEl.innerText : '';
        return {
            tokenUsage: tokenText.slice(0, 200),
            hasChainOfThought: hasCoT,
            citationCount: citations.length,
            toolCallCount: toolCalls.length,
            messageCount: messages.length,
            modeText: modeText.slice(0, 100)
        };
    }""")


async def test_query(page, query, query_name, mode):
    test_name = f"{query_name}-{mode.lower().replace(' ', '-')}"
    print(f"\n=== {test_name} ===")
    print(f"  Query: {query}")

    # Navigate to AI chat fresh
    await page.goto(f"{BASE_URL}/ai/chat", wait_until="networkidle", timeout=30000)
    await asyncio.sleep(2)
    await screenshot(page, f"{test_name}-01-initial")

    # Select mode via ModeSelector
    # The mode selector is typically a button/dropdown in the input area
    mode_selected = False
    try:
        # Try clicking the mode selector area (usually shows current mode name)
        mode_btn = page.locator('[class*="mode-selector"], [class*="ModeSelector"]').first
        if await mode_btn.is_visible(timeout=3000):
            await mode_btn.click()
            await asyncio.sleep(1)
            await screenshot(page, f"{test_name}-02-mode-menu")

            # Click the target mode
            mode_option = page.locator(f'text="{mode}"').first
            if await mode_option.is_visible(timeout=3000):
                await mode_option.click()
                await asyncio.sleep(1)
                mode_selected = True
                print(f"  ✓ mode set to: {mode}")
    except Exception as e:
        print(f"  ⚠ mode selection failed: {e}")

    if not mode_selected:
        print(f"  ⚠ could not select mode '{mode}', continuing with default")

    await screenshot(page, f"{test_name}-03-before-send")

    # Type query into input
    input_el = page.locator('textarea, [class*="input-box"] textarea').first
    await input_el.fill(query)
    await asyncio.sleep(0.5)

    # Send via Enter or send button
    send_btn = page.locator('[class*="send-button"], button[class*="send"]').first
    try:
        if await send_btn.is_visible(timeout=2000):
            await send_btn.click()
        else:
            await input_el.press("Enter")
    except Exception:
        await input_el.press("Enter")

    print("  ⏳ waiting for response...")
    await asyncio.sleep(3)
    await screenshot(page, f"{test_name}-04-streaming")

    # Wait for completion
    await wait_for_stream_done(page, timeout_ms=180000)
    await screenshot(page, f"{test_name}-05-final")

    # Scroll to capture full response
    try:
        msg_list = page.locator('[class*="message-list"], [class*="MessageList"]').first
        await msg_list.evaluate("el => el.scrollTop = el.scrollHeight / 2")
        await asyncio.sleep(1)
        await screenshot(page, f"{test_name}-06-middle")
    except Exception:
        pass

    metrics = await extract_metrics(page)
    print(f"  metrics: {json.dumps(metrics, ensure_ascii=False)}")
    return metrics


async def main():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False, args=["--window-size=1400,900"])
        context = await browser.new_context(viewport={"width": 1400, "height": 900}, device_scale_factor=2)
        page = await context.new_page()

        # Login
        print("Logging in...")
        await page.goto(f"{BASE_URL}/login", wait_until="networkidle", timeout=30000)
        await asyncio.sleep(2)

        # Fill credentials
        username_input = page.locator('input[placeholder*="用户名"], input[name="username"]').first
        password_input = page.locator('input[placeholder*="密码"], input[name="password"]').first
        await username_input.fill("demouser")
        await password_input.fill("DemoPass123")
        await asyncio.sleep(0.5)
        await screenshot(page, "00-login-filled")

        # Click login button
        login_btn = page.locator('button:has-text("下一步"), button:has-text("登录")').first
        await login_btn.click()

        await page.wait_for_load_state("networkidle", timeout=30000)
        await asyncio.sleep(3)
        await screenshot(page, "00-after-login")

        # Check if we're logged in
        current_url = page.url
        print(f"  After login URL: {current_url}")

        if "login" in current_url:
            print("  ⚠ still on login page, checking for errors...")
            error_text = await page.evaluate("() => document.body.innerText.slice(0, 500)")
            print(f"  Page text: {error_text}")
            await browser.close()
            return

        # Run tests
        results = []
        for query, query_name in QUERIES:
            for mode in MODES:
                try:
                    metrics = await test_query(page, query, query_name, mode)
                    results.append({"query": query, "mode": mode, "metrics": metrics})
                except Exception as e:
                    print(f"  ✗ error: {e}")
                    results.append({"query": query, "mode": mode, "error": str(e)})

        # Summary
        print("\n=== SUMMARY ===")
        print(json.dumps(results, ensure_ascii=False, indent=2))

        await browser.close()


if __name__ == "__main__":
    asyncio.run(main())
