// Child app i18n — zh-CN
// Includes child-specific keys and error code mappings from @numina/auth

export default {
  common: {
    confirm: '确认',
    cancel: '取消',
    back: '返回',
    loading: '加载中...',
    error: '出错了',
    retry: '重试',
    delete: '删除',
    edit: '编辑',
    pullRefresh: {
      pulling: '下拉即可刷新',
      loosing: '释放即可刷新',
      loading: '加载中...',
      success: '刷新成功',
    },
    coinUnit: {
      gold: '金',
      silver: '银',
      copper: '铜',
    },
  },
  nav: {
    home: '首页',
    tasks: '任务',
    ledger: '账本',
    wishes: '心愿',
    treasures: '宝藏',
  },
  auth: {
    selectChild: '选择小朋友',
    enterPin: '输入密码',
    enterParentPassword: '请输入家长密码',
    bindTitle: '绑定账号',
    childLogin: '儿童登录',
    username: '用户名',
    usernamePlaceholder: '请输入用户名',
    usernameRequired: '请输入用户名',
    password: '密码',
    passwordPlaceholder: '请输入密码',
    passwordRequired: '请输入密码',
    nextStep: '下一步',
    useFingerprint: '使用面容或指纹解锁',
    verifying: '验证中...',
    unlock: '🔓 解锁',
    useEmojiPin: '使用图形密码',
    useFaceId: '使用面容/指纹',
    backToLogin: '返回重新登录',
    deleteEmoji: '删除',
    clearPin: '清除',
  },
  errors: {
    PIN_ERROR: '❌ PIN错误，请重试',
    ACCOUNT_LOCKED: '🔒 账号已锁定，请让爸爸妈妈帮你解锁',
    INVALID_CREDENTIALS: '❌ 用户名或密码错误',
    AUTH_INVALID_CREDENTIALS: '❌ 用户名或密码错误',
    AUTH_CHILD_NOT_FOUND: '❌ 孩子不存在',
    AUTH_PIN_LOCKED: '🔒 PIN已锁定，请稍后再试',
    CHILD_NOT_FOUND: '❌ 孩子不存在',
    COIN_INSUFFICIENT: '⚠️ 余额不足',
    WISH_INSUFFICIENT_COINS: '⚠️ 积分不足，无法兑现',
    CHORE_ALREADY_CLAIMED: '⚠️ 任务已被认领',
    CHORE_INSTANCE_STATUS_CONFLICT: '⚠️ 任务状态已变更，请刷新',
    CHORE_NOT_YOURS: '⚠️ 这不是你的任务',
    wrongPassword: '❌ 密码错误，请重试',
    LOAD_FAILED: '❌ 加载失败，请刷新重试',
  },
  toast: {
    loginSuccess: '✅ 登录成功',
    logoutSuccess: '✅ 已退出',
    loginFailedGeneric: '❌ 登录失败，请重试',
    noPasskey: '⚠️ 未注册面容/指纹，请使用图形密码',
    verifyFailed: '❌ 验证失败，请重试',
    loadFailed: '❌ 加载失败，请刷新重试',
    submitFailed: '出了点小问题，请重试',
    drawFailed: '抽奖没成功，请稍后再试',
    useBonusFailed: '使用没成功，请稍后再试',
    noBonusDraws: '⚠️ 暂无免费抽奖机会，完成任务后可获得',
    grantBalanceFailed: '赠送没成功，请检查余额',
    childGrantedStars: '✅ 已赠送 {amount} ⭐ 给 {name}',
  },
  chore: {
    prevDay: '前一天',
    nextDay: '后一天',
    today: '今天',
    complete: '完成',
    pendingApproval: '审批中',
    approved: '✅ 已获得',
    rejected: '❌ 被拒绝',
    noChoresTitle: '今天没有家务 🎉',
    loadFailed: '❌ 加载失败，请刷新重试',
    submitFailed: '出了点小问题，请重试',
    streakBonus: '连击+{bonus}',
    claim: '认领',
    claiming: '认领中...',
    completeTitle: '确定完成这个任务吗？',
    completeConfirm: '确认完成',
    completeCancel: '再想想',
    abandon: '放弃',
    abandonLink: '不想做了？',
    abandonTitle: '确定放弃这个任务吗？',
    abandonKeepGoing: '继续完成',
    abandonConfirm: '放弃任务',
    abandonWishHint: '距离「{wishName}」还差 {remaining} ⭐，完成这个任务又近一步了 🌟',
    claimFailed: '认领没成功，任务可能已被认领',
    swipeToComplete: '右滑完成',
    consolation: '下次一定！继续努力',
    claimDisabledHistorical: '⏳ 历史任务无法认领',
    claimDisabledUnavailable: '🔒 当前状态无法认领',
    abandonFailed: '放弃没成功，请重试',
    wishProgressBump: '✨ +{stars} ⭐ 离「{wishName}」又近一步!',
    daysToBonus: '🔥 还差 {days} 天获得连击奖励!',
  },
  challenge: {
    taskCount: '累计任务',
    streakLength: '连续打卡',
    specificChore: '指定家务',
    starEarnings: '星币累计',
    progress: '进度',
    daysLeft: '剩余 {n} 天',
    completed: '🎉 挑战完成！',
    expired: '挑战已过期',
    active: '进行中',
    emptyState: '暂无挑战任务',
  },
  home: {
    greetingPhrases: [
      '你好呀 {name}！攒了 {balance} 颗星星 ⭐',
      '探险家 {name}，星库已同步 ⭐ {balance}',
      '欢迎回来，{name}！今天已有 {balance} ⭐',
      '{name} 的小金库：{balance} ⭐ 继续加油！',
      '{name} 的星星银行：{balance} ⭐',
    ],
    greetingFallback: '你好呀 {name}！攒了 {balance} 颗星星 ⭐',
    greetingFallbackName: '小探险家',
    todayTasks: '📋 今日任务',
    viewAllTasks: '全部任务',
    coinTiersShow: '看零钱',
    coinTiersHide: '收起',
    noTasks: '今天没有任务，好好休息吧！',
    myWishes: '我的心愿',
    myCalendar: '📅 我的日历',
    scenarioEntry: '📖 本周情境',
    scenarioEntrySub: '做出选择，解锁徽章',
    quickWishes: '我的心愿',
    quickTreasures: '我的宝贝',
    quickTasks: '所有任务',
    quickLedger: '星星账本',
    wishComplete: '% 完成',
    wishReady: ' · 可以兑现啦 🎉',
    wishWaitingGoal: '等待爸妈设定目标 ⏳',
    settings: '⚙️ 设置',
    settingsTheme: '主题模式',
    settingsLanguage: '语言',
    langZhCN: '🇨🇳 中文',
    langEnUS: '🇺🇸 English',
    themeSystem: '🌓 跟随系统',
    themeLight: '☀️ 日间模式',
    themeDark: '🌙 夜间模式',
    logout: '👋 退出登录',
    logoutConfirm: '⚠️ 确定要退出登录吗？',
    progressRingNoTasks: '今天没有任务，好好休息吧！',
    progressRingSubtitle: '还差 {remaining} 颗 ⭐ 就全部完成啦',
    progressRingAllDone: '今天的任务全部完成啦！🎉',
    progressRingAriaLabel: '今日任务进度：已完成 {completed} 项，共 {total} 项',
  },
  ledger: {
    giftBtn: '🎁 送给兄弟姐妹',
    empty: '还没有记录，快去完成家务吧！',
    sheetTitle: '🎁 送星星币',
    amountLabel: '数量',
    amountPlaceholder: '输入星星币数量',
    confirmGift: '确认赠送',
    confirmGiftWithRemaining: '确认赠送（转出后剩 {remaining} ⭐）',
    giftRemaining: '转出后你还剩 {remaining} ⭐',
    giftInsufficient: '余额不足，最多可转 {max} ⭐',
    txType: {
      parent_grant: '零花钱',
      wish_spend: '兑换心愿',
      chore_reward: '家务奖励',
      streak_bonus: '连续奖励',
      milestone_bonus: '里程碑奖励',
      sibling_gift: '兄弟姐妹赠送',
      adjustment: '余额调整',
    },
  },
  wishes: {
    starUnit: '⭐ 星星币',
    activeCount: '进行中',
    allWishes: '全部心愿',
    sectionActive: '✨ 进行中',
    sectionRedemption: '🎁 兑现申请中',
    sectionPending: '⏳ 审核中',
    sectionRealized: '🎊 已实现',
    sectionRejected: '❌ 未通过',
    progressFull: '积分已够！快让爸妈实现吧 🎉',
    progressDays: '按最近速度，大约还要做 {days} 天家务 🎯',
    waitingGoal: '⏳ 等待爸妈设定目标',
    redeemBtn: '🎉 让爸妈实现！',
    waitingRedemption: '等待爸妈兑现 🎁',
    waitingReview: '等待爸妈审核 ⏳',
    realized: '已实现 ✅',
    rejected: '未通过 ❌',
    emptyText: '还没有心愿，快许个愿吧！',
    createBtn: '+ 许个愿',
    sheetTitle: '🌟 许个愿',
    wishNameLabel: '心愿名称',
    wishNamePlaceholder: '最多50字',
    emojiLabel: '表情',
    emojiPlaceholder: '可选，如 🎮 🚲',
    descLabel: '描述',
    descPlaceholder: '可选，最多200字',
    priorityLabel: '有多想要',
    submitBtn: '提交心愿',
    priorityHigh: '最想要 🔥',
    priorityMedium: '比较想 ⭐',
    priorityLow: '以后再说 💤',
    priorityLabelHigh: '最想要 🔥',
    priorityLabelMedium: '比较想 ⭐',
    priorityLabelLow: '以后再说 💤',
    createPageTitle: '许个愿 🌟',
    submitSuccess: '✅ 心愿已提交！',
    continueCreate: '继续许愿',
    backToList: '返回心愿列表',
    emojiPickerTitle: '选择表情',
    tint: {
      green: { aria: '可以兑换啦' },
      yellow: { aria: '快可以兑换了' },
      red: { aria: '还要再等一阵子' },
      gray: { aria: '继续做家务，几天后能更准估计' },
    },
    constellation: {
      headline: '你今天可以拿到 {k} 个 / 共 {n} 个 心愿',
      headlineZero: '继续加油，离最近的心愿还差 {d} 天',
      headlineZeroNoEstimate: '继续做家务，慢慢攒星星',
      detailUnknown: '没找到这个心愿',
    },
    timeUnitDays: '按最近速度约 {days} 天',
    timeUnitPlaceholder: '继续做家务，几天后能更准估计',
    peek: {
      confirmTag: '这个就能拿到啦 ✨',
      daysAdded: '+{n} 天',
    },
    fulfilledAt: '兑现于 {date}',
    viewAsset: '查看宝贝 →',
  },
  treasures: {
    title: '🏆 我的宝贝',
    earnedCount: '你已经赚到了 {count} 件宝贝！',
    totalCoins: '共花费 {coins} 颗星 ⭐',
    empty: '还没有宝贝，快去完成家务赚星星币吧！',
  },
  blindBox: {
    navTitle: '🎁 盲盒抽奖',
    tabDraw: '抽奖',
    tabHistory: '历史',
    bonusDivider: '免费抽奖机会 ({count})',
    bonusListLabel: '免费抽奖机会列表',
    bonusItem: '🎀 免费抽奖（{expiry}到期）',
    bonusItemAriaLabel: '使用免费抽奖机会，{expiry}到期',
    useBtn: '使用',
    drawAgain: '再抽一次',
    tapHint: '点击抽奖',
    drawing: '抽奖中...',
    giftRevealLabel: '恭喜！抽到了 {name}',
    surpriseBadge: '✨ 超预期惊喜！',
    bonusBadge: '🎀 免费抽奖',
    historyEmpty: '暂无抽奖记录',
    historyAriaLabel: '抽奖历史',
    coinsSpent: '花费 {coins} 金币',
    surpriseLabel: '✨ 惊喜',
    bonusLabel: '🎀 免费',
    statusFulfilled: '已兑现',
    statusPending: '待兑现',
    surpriseAriaPrefix: '超预期惊喜，',
    autoTriggeredClose: '关闭',
  },
  calendar: {
    legendChore: '打卡',
    legendWish: '心愿',
    legendMilestone: '成就',
    statsChores: '打卡 {count}次',
    statsWishes: '心愿 {count}个',
    statsMilestones: '成就 {count}个',
    statsRate: '完成率 {rate}%',
    monthTitle: '{year}年{month}月',
    weekdays: ['日', '一', '二', '三', '四', '五', '六'],
    prevMonth: '上个月',
    nextMonth: '下个月',
    dayLabel: '{month}月{day}日，{count}项活动',
  },
  dayDetail: {
    sectionChores: '📋 打卡任务',
    sectionWishes: '🌟 心愿实现',
    sectionMilestones: '🏆 成就解锁',
    statusApproved: '已完成',
    statusPending: '待审批',
    statusRealized: '已实现',
    statusMilestone: '新成就',
    streakBonus: '连击+{bonus}',
    wishCost: '花费 {cost} ⭐',
    empty: '这天没有记录',
    defaultTitle: '当日明细',
    dateTitle: '{month}月{day}日',
  },
  milestone: {
    first_chore: '完成第一个任务',
    first_wish_realized: '第一个心愿实现',
    coins_50: '累计获得50⭐',
    coins_200: '累计获得200⭐',
    streak_7: '连续打卡7天',
    streak_14: '连续打卡14天',
    streak_30: '连续打卡30天',
    titles: {
      first_chore: '第一个家务！',
      first_wish_realized: '第一个心愿实现！',
      coins_50: '攒到50颗星星币！',
      coins_200: '攒到200颗星星币！',
      streak_7: '连续打卡7天！',
      streak_14: '连续打卡14天！',
      streak_30: '连续打卡30天！',
      default: '新成就！',
    },
    descs: {
      first_chore: '你完成了人生第一个家务，太棒了！',
      first_wish_realized: '你的努力换来了心愿成真！',
      coins_50: '继续加油，更多心愿等着你！',
      coins_200: '你是理财小达人！',
      streak_7: '一周不间断，获得1.5倍奖励！',
      streak_14: '两周坚持，获得2倍奖励！',
      streak_30: '一个月的坚持，你是冠军！',
      default: '恭喜你解锁了新成就！',
    },
    icons: {
      first_chore: '🌟',
      first_wish_realized: '🎁',
      coins_50: '💰',
      coins_200: '🏆',
      streak_7: '🔥',
      streak_14: '🔥🔥',
      streak_30: '👑',
      default: '🎉',
    },
    dismissBtn: '太棒了！',
    overlayLabel: '成就解锁',
  },
  celebration: {
    phrases: [
      '太棒了！',
      '厉害！',
      '真行！',
      '继续加油！',
      '做得好！',
      '你真棒！',
      '加油加油！',
    ],
    singleTask: '获得 {stars} ⭐！',
    multipleTasks: '{count}个任务通过！获得 {stars} ⭐',
    overlayLabel: '任务通过庆祝',
    treasureUnlocked: '宝藏解锁！',
    confirmButton: '太棒了！',
    sealTreasureChest: '锁住宝箱！',
    reducedMotionToast: '✨ 任务通过！获得 {stars} ⭐',
    candleAriaLabel: '等待审批中',
    educationReward: '爸爸妈妈奖励了你 {coins} 星星币！',
  },
  assetDetail: {
    title: '资产详情',
    purchaseDate: '购入日期',
    purchasePrice: '购入价格',
    currentValue: '当前价值',
  },
  scenario: {
    title: '本周情境',
    loading: '加载情境中...',
    completed: '已完成 ✅',
    choosePrompt: '你会怎么做？',
    feedbackTitle: '你的选择',
    dimension: '这关乎',
    badgesUnlocked: '解锁了新徽章',
    close: '好的',
    alreadyCompleted: '本周的情境已完成',
    loadFailed: '加载失败，请刷新重试',
    submitFailed: '出了点小问题，请重试',
    dimensions: {
      earning: '赚钱',
      choosing: '选择',
      waiting: '等待',
      caring: '关心',
    },
  },
  badges: {
    title: '素养徽章',
    loading: '加载中...',
    entryTitle: '素养徽章',
    entryCount: '{count} 枚徽章',
    entryEmpty: '还没有徽章',
    dimensions: {
      earning: '赚钱',
      choosing: '选择',
      waiting: '等待',
      caring: '关心',
    },
    currentBadge: '当前徽章',
    nextBadge: '下一枚',
    history: '历史',
    surpassed: '超越',
    locked: '未解锁',
    level: '等级',
    criteria: '解锁条件',
    empty: '还没有徽章，完成情境游戏来解锁吧！',
    earnedAt: '获得于',
  },
  manifesto: {
    title: '家庭约定',
    sign: '签署约定',
    tapConsent: '同意',
    confirmConsent: '你确定要同意吗？',
    handwriting: '手写签名',
    hideHandwriting: '收起手写',
    stamp: '盖章',
    signSuccess: '签署成功！',
    ourManifesto: '我们家的约定',
    coCreated: '共同约定',
    companionDays: '我们的约定已经陪伴我们 {days} 天了',
    goToSign: '前往签署',
    guardian: '家庭约定守护者',
    popupHint: '家人有一份新的约定等你签署哦',
    signHint: '在下方写下你的名字吧',
    notFound: '暂时没有家庭约定',
    trackableHint: '这个任务和我们的家庭约定有关哦',
  },
  childOnboarding: {
    skip: '跳过',
    next: '下一步',
    done: '知道了',
    step1: {
      empty: { title: '你的家务任务', desc: '这里会显示你的家务任务，完成后就能获得奖励' },
      data: { title: '你的任务列表', desc: '完成家务就能获得奖励' },
    },
    step2: { title: '我的奖励', desc: '积攒奖励，兑换心愿' },
  },
  learning: {
    title: '知识地图',
    empty: '还没有学习内容，让爸爸妈妈帮你布置吧！',
    todayRecommend: '今日推荐',
    startLearning: '🎓 开始学习',
    submitAssignment: '📝 提交审核',
    submitSuccess: '✅ 已提交审核',
    noPendingAssignment: '⚠️ 没有待提交的任务',
    attempts: '{count} 次尝试',
    evidence: '学习目标',
    general: '综合',
    subject: {
      mathematics: '数学',
      science: '科学',
      english: '英语',
      history: '历史',
      personal_social: '个人与社会',
      life_skills: '生活技能',
      computing: '计算机',
      learning_to_learn: '学会学习',
    },
    status: {
      available: '可学习',
      learning: '学习中',
      mastered: '已掌握',
      locked: '未解锁',
      review: '待复习',
    },
    session: {
      chatTitle: 'AI 导师对话进行中',
      chatDesc: '和 AI 导师一起学习这个话题吧',
      noThread: '正在准备学习会话...',
      finish: '✅ 完成学习',
      ended: '学习已完成！',
      streaming: '正在回答...',
      completed: '已完成',
      inputPlaceholder: '输入你的回答...',
      send: '发送',
    },
    error: {
      stream: '流式连接错误',
      gap_recovery: '事件流中断，正在重新连接…',
      session_interrupted: '会话已{status}',
      assessment_stream: '评估流连接失败: {status}',
      reconnect: '重新连接失败: {status}',
      send: '发送消息失败: {status}',
      gap_reconnect: '断点恢复失败: {status}',
    },
    progress: {
      title: '学习进度',
      studyTime: '学习时长',
      minutes: '分钟',
      recentActivity: '最近活动',
      noActivity: '还没有学习记录，开始学习吧！',
      comingSoon: '更多学习统计即将推出...',
      totalMinutes: '分钟（总计）',
      todayMinutes: '分钟（今日）',
    },
    todayCard: {
      assignment: '提交作业',
      current: '继续学习',
      recommended: '开始新知识',
      explore: '去学习地图探索',
      exploreSub: '探索知识地图',
      footer: '今日已学习 {minutes} 分钟',
    },
    prerequisites: '前置知识',
    nextSteps: '后续学习',
  },
  empty: {
    noTasks: '今天没有任务，好好休息吧 🌟',
    allDone: '今天的任务都完成啦！太棒了 🎉',
    noWishes: '许个心愿吧，努力就能实现！✨',
    noTreasures: '完成心愿后，宝贝会出现在这里 🎁',
    noRecords: '开始做任务，星星币就会到账 💫',
  },
  profileEdit: {
    title: '编辑资料',
    tapToChange: '点击头像更换',
    displayName: '昵称',
    displayNamePlaceholder: '请输入昵称',
    displayNameRequired: '昵称不能为空',
  },
  avatarPicker: {
    title: '选择头像',
    tab3d: '3D 图标',
    tabEmoji: '表情',
    emojiHint: '点击下方输入框选择表情',
    emojiPlaceholder: '点击输入表情',
  },
  pwa: {
    installPrompt: '添加到主屏幕，获得更好的体验',
    install: '添加',
    dismiss: '稍后',
    offlineData: '离线数据 · 缓存于 {time}前',
    firstTimeOffline: '首次使用需联网加载数据',
    syncing: '正在同步 {count} 条操作...',
    pendingSync: '{count} 条操作待同步',
    syncComplete: '同步完成',
    syncConflict: '该记录已被更新，本地修改已跳过',
    syncFailed: '同步失败，请重试',
    syncFailedCount: '{count} 条同步失败',
    retry: '重试',
    discard: '丢弃',
    pushTitle: '推送通知',
    pushDescription: '接收提醒和家庭互动通知',
    pushEnable: '开启推送',
    pushDisable: '关闭推送',
    pushDeniedHint: '如需提醒可在系统设置中开启通知权限',
    pushUnsupported: '当前浏览器不支持推送通知',
    updateAvailable: '有新版本可用',
    updateNow: '立即更新',
  },
}
