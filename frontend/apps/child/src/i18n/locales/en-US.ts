// Child app i18n — en-US

export default {
  common: {
    confirm: 'Confirm',
    cancel: 'Cancel',
    back: 'Back',
    loading: 'Loading...',
    error: 'Error',
    retry: 'Retry',
    delete: 'Delete',
    edit: 'Edit',
    pullRefresh: {
      pulling: 'Pull to refresh',
      loosing: 'Release to refresh',
      loading: 'Loading...',
      success: 'Refreshed',
    },
    coinUnit: {
      gold: 'G',
      silver: 'S',
      copper: 'C',
    },
  },
  nav: {
    home: 'Home',
    tasks: 'Tasks',
    ledger: 'Ledger',
    wishes: 'Wishes',
    treasures: 'Treasures',
  },
  auth: {
    selectChild: 'Select Child',
    enterPin: 'Enter PIN',
    enterParentPassword: 'Enter Parent Password',
    bindTitle: 'Bind Account',
    childLogin: 'Child Login',
    username: 'Username',
    usernamePlaceholder: 'Enter username',
    usernameRequired: 'Please enter username',
    password: 'Password',
    passwordPlaceholder: 'Enter password',
    passwordRequired: 'Please enter password',
    nextStep: 'Next',
    useFingerprint: 'Use Face ID or Fingerprint',
    verifying: 'Verifying...',
    unlock: '🔓 Unlock',
    useEmojiPin: 'Use Emoji PIN',
    useFaceId: 'Use Face ID / Fingerprint',
    backToLogin: 'Back to Login',
    deleteEmoji: 'Delete',
    clearPin: 'Clear',
  },
  errors: {
    PIN_ERROR: '❌ Wrong PIN, please try again',
    ACCOUNT_LOCKED: '🔒 Account locked, please ask a parent to unlock',
    INVALID_CREDENTIALS: '❌ Invalid username or password',
    AUTH_INVALID_CREDENTIALS: '❌ Invalid username or password',
    AUTH_CHILD_NOT_FOUND: '❌ Child not found',
    AUTH_PIN_LOCKED: '🔒 PIN locked, please try again later',
    CHILD_NOT_FOUND: '❌ Child not found',
    COIN_INSUFFICIENT: '⚠️ Insufficient balance',
    WISH_INSUFFICIENT_COINS: '⚠️ Not enough coins to redeem',
    CHORE_ALREADY_CLAIMED: '⚠️ Task already claimed',
    CHORE_INSTANCE_STATUS_CONFLICT: '⚠️ Task status changed, please refresh',
    CHORE_NOT_YOURS: '⚠️ This is not your task',
    wrongPassword: '❌ Wrong password, please try again',
    LOAD_FAILED: '❌ Load failed, please refresh and try again',
  },
  toast: {
    loginSuccess: '✅ Logged in',
    logoutSuccess: '✅ Logged out',
    loginFailedGeneric: '❌ Login failed, please try again',
    noPasskey: '⚠️ No Face ID / Fingerprint registered, please use Emoji PIN',
    verifyFailed: '❌ Verification failed, please try again',
    loadFailed: '❌ Load failed, please refresh and try again',
    submitFailed: 'Something went wrong, please try again',
    drawFailed: 'Draw was not successful, please try again later',
    useBonusFailed: 'Bonus could not be used, please try again later',
    noBonusDraws: '⚠️ No free draws available, complete tasks to earn some',
    grantBalanceFailed: 'Gift was not successful, please check your balance',
    childGrantedStars: '✅ Gifted {amount} ⭐ to {name}',
  },
  chore: {
    prevDay: 'Previous Day',
    nextDay: 'Next Day',
    today: 'Today',
    complete: 'Complete',
    pendingApproval: 'Pending',
    approved: '✅ Earned',
    rejected: '❌ Rejected',
    noChoresTitle: 'No tasks today 🎉',
    loadFailed: '❌ Load failed, please refresh and try again',
    submitFailed: 'Something went wrong, please try again',
    streakBonus: 'Streak +{bonus}',
    claim: 'Claim',
    claiming: 'Claiming...',
    completeTitle: 'Mark this task as done?',
    completeConfirm: 'Yes, Done!',
    completeCancel: 'Not Yet',
    abandon: 'Give Up',
    abandonLink: 'Not feeling it?',
    abandonTitle: 'Give up this task?',
    abandonKeepGoing: 'Keep Going',
    abandonConfirm: 'Give Up Task',
    abandonWishHint: '{remaining} ⭐ away from 「{wishName}」 — finishing this gets you closer 🌟',
    claimFailed: 'Could not claim, task may already be taken',
    swipeToComplete: 'Swipe to complete',
    consolation: 'Better luck next time! Keep going',
    claimDisabledHistorical: '⏳ Past tasks cannot be claimed',
    claimDisabledUnavailable: '🔒 Cannot claim in current state',
    abandonFailed: 'Could not give up, please try again',
    wishProgressBump: '✨ +{stars} ⭐ closer to {wishName}!',
    daysToBonus: '🔥 {days} more days to streak bonus!',
  },
  challenge: {
    taskCount: 'Total Tasks',
    streakLength: 'Daily Streak',
    specificChore: 'Specific Task',
    starEarnings: 'Stars Earned',
    progress: 'Progress',
    daysLeft: '{n} days left',
    completed: '🎉 Challenge completed!',
    expired: 'Challenge expired',
    active: 'In Progress',
    emptyState: 'No challenges right now',
  },
  home: {
    greetingPhrases: [
      'Hi {name}! You\'ve saved {balance} stars ⭐',
      'Explorer {name}, star vault synced ⭐ {balance}',
      'Welcome back, {name}! {balance} ⭐ today',
      '{name}\'s little vault: {balance} ⭐ keep going!',
      '{name}\'s star bank: {balance} ⭐',
    ],
    greetingFallback: 'Hi {name}! You\'ve saved {balance} stars ⭐',
    greetingFallbackName: 'Explorer',
    todayTasks: '📋 Today\'s Tasks',
    viewAllTasks: 'All Tasks',
    coinTiersShow: 'See coins',
    coinTiersHide: 'Hide',
    noTasks: 'No tasks today, enjoy your rest!',
    myWishes: 'My Wishes',
    myCalendar: '📅 My Calendar',
    scenarioEntry: '📖 Weekly Scenario',
    scenarioEntrySub: 'Make a choice, unlock badges',
    quickWishes: 'My Wishes',
    quickTreasures: 'My Treasures',
    quickTasks: 'All Tasks',
    quickLedger: 'Star Ledger',
    wishComplete: '% complete',
    wishReady: ' · Ready to redeem 🎉',
    wishWaitingGoal: 'Waiting for parents to set a goal ⏳',
    settings: '⚙️ Settings',
    settingsTheme: 'Theme',
    settingsLanguage: 'Language',
    langZhCN: '🇨🇳 中文',
    langEnUS: '🇺🇸 English',
    themeSystem: '🌓 System',
    themeLight: '☀️ Light',
    themeDark: '🌙 Dark',
    logout: '👋 Log Out',
    logoutConfirm: '⚠️ Are you sure you want to log out?',
    progressRingNoTasks: 'No tasks today, enjoy your rest!',
    progressRingSubtitle: '{remaining} ⭐ left to finish them all',
    progressRingAllDone: 'All tasks done today! 🎉',
    progressRingAriaLabel: 'Today\'s task progress: {completed} of {total} done',
  },
  ledger: {
    giftBtn: '🎁 Gift to Sibling',
    empty: 'No records yet, go complete some tasks!',
    sheetTitle: '🎁 Gift Star Coins',
    amountLabel: 'Amount',
    amountPlaceholder: 'Enter amount',
    confirmGift: 'Confirm Gift',
    confirmGiftWithRemaining: 'Confirm ({remaining} ⭐ left after)',
    giftRemaining: 'You\'ll have {remaining} ⭐ left after',
    giftInsufficient: 'Not enough stars — max {max} ⭐',
    txType: {
      parent_grant: 'Allowance',
      wish_spend: 'Wish Redeemed',
      chore_reward: 'Task Reward',
      streak_bonus: 'Streak Bonus',
      milestone_bonus: 'Milestone Bonus',
      sibling_gift: 'Sibling Gift',
      adjustment: 'Balance Adjustment',
    },
  },
  wishes: {
    starUnit: '⭐ Star Coins',
    activeCount: 'Active',
    allWishes: 'All Wishes',
    sectionActive: '✨ Active',
    sectionRedemption: '🎁 Redemption Requested',
    sectionPending: '⏳ Pending Review',
    sectionRealized: '🎊 Realized',
    sectionRejected: '❌ Rejected',
    progressFull: 'Enough coins! Ask your parents to make it happen 🎉',
    progressDays: 'At your recent pace, about {days} more days of tasks 🎯',
    waitingGoal: '⏳ Waiting for parents to set a goal',
    redeemBtn: '🎉 Ask Parents to Redeem!',
    waitingRedemption: 'Waiting for parents to redeem 🎁',
    waitingReview: 'Waiting for parents to review ⏳',
    realized: 'Realized ✅',
    rejected: 'Rejected ❌',
    emptyText: 'No wishes yet, make a wish!',
    createBtn: '+ Make a Wish',
    sheetTitle: '🌟 Make a Wish',
    wishNameLabel: 'Wish Name',
    wishNamePlaceholder: 'Up to 50 characters',
    emojiLabel: 'Emoji',
    emojiPlaceholder: 'Optional, e.g. 🎮 🚲',
    descLabel: 'Description',
    descPlaceholder: 'Optional, up to 200 characters',
    priorityLabel: 'How much I want it',
    submitBtn: 'Submit Wish',
    priorityHigh: 'Want most 🔥',
    priorityMedium: 'Kind of want ⭐',
    priorityLow: 'Maybe later 💤',
    priorityLabelHigh: 'Want most 🔥',
    priorityLabelMedium: 'Kind of want ⭐',
    priorityLabelLow: 'Maybe later 💤',
    createPageTitle: 'Make a Wish 🌟',
    submitSuccess: '✅ Wish submitted!',
    continueCreate: 'Make Another',
    backToList: 'Back to Wishes',
    emojiPickerTitle: 'Pick an Emoji',
    tint: {
      green: { aria: 'Ready to claim' },
      yellow: { aria: 'Almost ready' },
      red: { aria: 'Still saving' },
      gray: { aria: 'Keep saving — estimate stabilizes after a few days' },
    },
    constellation: {
      headline: 'You can get {k} of {n} wishes today',
      headlineZero: 'Keep going — closest wish is {d} days away',
      headlineZeroNoEstimate: 'Keep doing chores to grow your stars',
      detailUnknown: 'This wish was not found',
    },
    timeUnitDays: 'about {days} days at your pace',
    timeUnitPlaceholder: 'Keep going — estimate stabilizes',
    peek: {
      confirmTag: 'You can have this one ✨',
      daysAdded: '+{n} days',
    },
    fulfilledAt: 'Fulfilled on {date}',
    viewAsset: 'View Treasure →',
  },
  treasures: {
    title: '🏆 My Treasures',
    earnedCount: 'You\'ve earned {count} treasures!',
    totalCoins: 'Total {coins} stars spent ⭐',
    empty: 'No treasures yet, complete tasks to earn Star Coins!',
  },
  blindBox: {
    navTitle: '🎁 Lucky Draw',
    tabDraw: 'Draw',
    tabHistory: 'History',
    bonusDivider: 'Free Draws ({count})',
    bonusListLabel: 'Free draw list',
    bonusItem: '🎀 Free Draw (expires {expiry})',
    bonusItemAriaLabel: 'Use free draw, expires {expiry}',
    useBtn: 'Use',
    drawAgain: 'Draw Again',
    tapHint: 'Tap to Draw',
    drawing: 'Drawing...',
    giftRevealLabel: 'Congrats! You got {name}',
    surpriseBadge: '✨ Surprise!',
    bonusBadge: '🎀 Free Draw',
    historyEmpty: 'No draw history yet',
    historyAriaLabel: 'Draw history',
    coinsSpent: 'Spent {coins} coins',
    surpriseLabel: '✨ Surprise',
    bonusLabel: '🎀 Free',
    statusFulfilled: 'Fulfilled',
    statusPending: 'Pending',
    surpriseAriaPrefix: 'Surprise, ',
    autoTriggeredClose: 'Close',
  },
  calendar: {
    legendChore: 'Task',
    legendWish: 'Wish',
    legendMilestone: 'Achievement',
    statsChores: '{count} tasks',
    statsWishes: '{count} wishes',
    statsMilestones: '{count} achievements',
    statsRate: '{rate}% completion',
    monthTitle: '{year}/{month}',
    weekdays: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
    prevMonth: 'Previous month',
    nextMonth: 'Next month',
    dayLabel: '{month}/{day}, {count} activities',
  },
  dayDetail: {
    sectionChores: '📋 Tasks',
    sectionWishes: '🌟 Wishes Realized',
    sectionMilestones: '🏆 Achievements Unlocked',
    statusApproved: 'Completed',
    statusPending: 'Pending',
    statusRealized: 'Realized',
    statusMilestone: 'New Achievement',
    streakBonus: 'Streak +{bonus}',
    wishCost: 'Spent {cost} ⭐',
    empty: 'No records for this day',
    defaultTitle: 'Day Detail',
    dateTitle: '{month}/{day}',
  },
  milestone: {
    first_chore: 'Completed first task',
    first_wish_realized: 'First wish realized',
    coins_50: 'Earned 50 ⭐ total',
    coins_200: 'Earned 200 ⭐ total',
    streak_7: '7-day streak',
    streak_14: '14-day streak',
    streak_30: '30-day streak',
    titles: {
      first_chore: 'First Task Done!',
      first_wish_realized: 'First Wish Realized!',
      coins_50: 'Saved 50 Star Coins!',
      coins_200: 'Saved 200 Star Coins!',
      streak_7: '7-Day Streak!',
      streak_14: '14-Day Streak!',
      streak_30: '30-Day Streak!',
      default: 'New Achievement!',
    },
    descs: {
      first_chore: 'You completed your very first task, amazing!',
      first_wish_realized: 'Your hard work made a wish come true!',
      coins_50: 'Keep going, more wishes are waiting!',
      coins_200: 'You\'re a saving superstar!',
      streak_7: 'One week straight, earning 1.5× rewards!',
      streak_14: 'Two weeks strong, earning 2× rewards!',
      streak_30: 'A whole month of effort, you\'re a champion!',
      default: 'Congratulations on unlocking a new achievement!',
    },
    icons: {
      first_chore: '🌟',
      first_wish_realized: '🎁',
      coins_50: '💰',
      coins_200: '🏆',
      streak_7: '🔥',
      streak_14: '🔥🔥',
      streak_30: '👑',
      default: '🎉',
    },
    dismissBtn: 'Awesome!',
    overlayLabel: 'Achievement Unlocked',
  },
  celebration: {
    phrases: [
      'Awesome!',
      'Great job!',
      'Amazing!',
      'Keep it up!',
      'Well done!',
      'You\'re amazing!',
      'Go, go, go!',
    ],
    singleTask: 'Earned {stars} ⭐!',
    multipleTasks: '{count} tasks approved! Earned {stars} ⭐',
    overlayLabel: 'Task approval celebration',
    treasureUnlocked: 'Treasure Unlocked!',
    confirmButton: 'Awesome!',
    sealTreasureChest: 'Seal the Treasure Chest!',
    reducedMotionToast: '✨ Task approved! Earned {stars} ⭐',
    candleAriaLabel: 'Awaiting approval',
    educationReward: 'Mom and Dad rewarded you {coins} star coins!',
  },
  assetDetail: {
    title: 'Asset Details',
    purchaseDate: 'Purchase Date',
    purchasePrice: 'Purchase Price',
    currentValue: 'Current Value',
  },
  scenario: {
    title: 'This Week\'s Scenario',
    loading: 'Loading scenario...',
    completed: 'Completed ✅',
    choosePrompt: 'What would you do?',
    feedbackTitle: 'Your Choice',
    dimension: 'This is about',
    badgesUnlocked: 'Unlocked a new badge',
    close: 'OK',
    alreadyCompleted: 'This week\'s scenario is complete',
    loadFailed: 'Load failed, please refresh and try again',
    submitFailed: 'Something went wrong, please try again',
    dimensions: {
      earning: 'Earning',
      choosing: 'Choosing',
      waiting: 'Waiting',
      caring: 'Caring',
    },
  },
  badges: {
    title: 'Literacy Badges',
    loading: 'Loading...',
    entryTitle: 'Literacy Badges',
    entryCount: '{count} badges',
    entryEmpty: 'No badges yet',
    dimensions: {
      earning: 'Earning',
      choosing: 'Choosing',
      waiting: 'Waiting',
      caring: 'Caring',
    },
    currentBadge: 'Current Badge',
    nextBadge: 'Next',
    history: 'History',
    surpassed: 'Surpassed',
    locked: 'Locked',
    level: 'Level',
    criteria: 'Unlock criteria',
    empty: 'No badges yet — complete scenario games to unlock!',
    earnedAt: 'Earned on',
  },
  manifesto: {
    title: 'Family Manifesto',
    sign: 'Sign Manifesto',
    tapConsent: 'I Agree',
    confirmConsent: 'Are you sure you want to agree?',
    handwriting: 'Handwrite signature',
    hideHandwriting: 'Hide handwriting',
    stamp: 'Seal',
    signSuccess: 'Signed!',
    ourManifesto: 'Our Family Manifesto',
    coCreated: 'co-created',
    companionDays: 'Our manifesto has been with us for {days} days',
    goToSign: 'Go to sign',
    guardian: 'Family Manifesto Guardian',
    popupHint: 'Your family has a new manifesto waiting for you to sign',
    signHint: 'Write your name below',
    notFound: 'No family manifesto right now',
    trackableHint: 'This task is related to our family manifesto',
  },
  childOnboarding: {
    skip: 'Skip',
    next: 'Next',
    done: 'Got it',
    step1: {
      empty: { title: 'Your Chores', desc: 'Your tasks will appear here. Complete them to earn rewards' },
      data: { title: 'Your Tasks', desc: 'Complete chores to earn rewards' },
    },
    step2: { title: 'My Rewards', desc: 'Save up rewards to redeem wishes' },
  },
  learning: {
    title: 'Knowledge Map',
    empty: 'No learning content yet — ask your parents to assign some!',
    todayRecommend: 'Today\'s Pick',
    startLearning: '🎓 Start Learning',
    submitAssignment: '📝 Submit for Review',
    submitSuccess: '✅ Submitted for review',
    noPendingAssignment: '⚠️ No pending assignment',
    attempts: '{count} attempts',
    evidence: 'Learning Goals',
    general: 'General',
    subject: {
      mathematics: 'Math',
      science: 'Science',
      english: 'English',
      history: 'History',
      personal_social: 'Personal & Social',
      life_skills: 'Life Skills',
      computing: 'Computing',
      learning_to_learn: 'Learning to Learn',
    },
    status: {
      available: 'Available',
      learning: 'Learning',
      mastered: 'Mastered',
      locked: 'Locked',
      review: 'Review',
    },
    session: {
      chatTitle: 'AI Tutor Session in Progress',
      chatDesc: 'Learn with your AI tutor about this topic',
      noThread: 'Preparing your learning session...',
      finish: '✅ Finish Learning',
      ended: 'Learning session completed!',
      streaming: 'Answering...',
      completed: 'Completed',
      inputPlaceholder: 'Type your answer...',
      send: 'Send',
    },
    error: {
      stream: 'Stream connection error',
      gap_recovery: 'Stream interrupted, reconnecting…',
      session_interrupted: 'Session {status}',
      assessment_stream: 'Assessment stream failed: {status}',
      reconnect: 'Reconnect failed: {status}',
      send: 'Send message failed: {status}',
      gap_reconnect: 'Gap recovery failed: {status}',
    },
    progress: {
      title: 'Learning Progress',
      studyTime: 'Study Time',
      minutes: 'min',
      recentActivity: 'Recent Activity',
      noActivity: 'No learning records yet — start learning!',
      comingSoon: 'More learning stats coming soon...',
      totalMinutes: 'min (total)',
      todayMinutes: 'min (today)',
    },
    todayCard: {
      assignment: 'Submit Assignment',
      current: 'Continue Learning',
      recommended: 'Start New Topic',
      explore: 'Explore Learning Map',
      exploreSub: 'Explore the knowledge map',
      footer: 'Studied {minutes} min today',
    },
    prerequisites: 'Prerequisites',
    nextSteps: 'Next Steps',
  },
  empty: {
    noTasks: 'No tasks today, enjoy your rest 🌟',
    allDone: 'All done for today! Amazing 🎉',
    noWishes: 'Make a wish — hard work makes it happen! ✨',
    noTreasures: 'Treasures will appear here after wishes come true 🎁',
    noRecords: 'Start doing tasks and Star Coins will roll in 💫',
  },
  profileEdit: {
    title: 'Edit Profile',
    tapToChange: 'Tap avatar to change',
    displayName: 'Display Name',
    displayNamePlaceholder: 'Enter display name',
    displayNameRequired: 'Display name is required',
  },
  avatarPicker: {
    title: 'Choose Avatar',
    tab3d: '3D Icons',
    tabEmoji: 'Emoji',
    emojiHint: 'Tap the input below to select an emoji',
    emojiPlaceholder: 'Tap to enter emoji',
  },
  pwa: {
    installPrompt: 'Add to home screen for a better experience',
    install: 'Add',
    dismiss: 'Later',
    offlineData: 'Offline data · cached {time} ago',
    firstTimeOffline: 'Internet connection required for first load',
    syncing: 'Syncing {count} operations...',
    pendingSync: '{count} operations pending sync',
    syncComplete: 'Sync complete',
    syncConflict: 'Record was updated, local changes skipped',
    syncFailed: 'Sync failed, please retry',
    syncFailedCount: '{count} sync failed',
    retry: 'Retry',
    discard: 'Discard',
    pushTitle: 'Push Notifications',
    pushDescription: 'Receive reminders and family notifications',
    pushEnable: 'Enable Push',
    pushDisable: 'Disable Push',
    pushDeniedHint: 'Enable notifications in system settings to receive reminders',
    pushUnsupported: 'Push notifications are not supported in this browser',
    updateAvailable: 'New version available',
    updateNow: 'Update now',
  },
}
