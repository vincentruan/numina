      ...
      <div class="provider-sheet__list">
        ...
        >
          <div class="provider-item__icon" :class="`provider-item__icon--${provider.value}`">
            ...
            <!-- Gemini sparkle logo (Google blue) -->
            <svg v-else-if="provider.value === 'gemini'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 2L14.39 8.66L21 11L14.39 13.34L12 20L9.61 13.34L3 11L9.61 8.66L12 2Z" fill="currentColor" />
              <circle cx="18.5" cy="5.5" r="2" fill="currentColor" opacity="0.6" />
              <circle cx="5.5" cy="18" r="1.5" fill="currentColor" opacity="0.4" />
            </svg>
            <!-- OpenAI Compatible logo (blue-gray, with plug badge) -->
            <svg v-else-if="provider.value === 'openai_compatible'" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const providers = computed<ProviderOption[]>(() => [
  ...
  {
    value: 'gemini',
    label: t('aiConfig.providerGemini'),
    subtitle: 'generateContent endpoint',
  },
])
...
/* Gemini: Google blue */
.provider-item__icon--gemini {
  background: color-mix(in srgb, #4285f4 12%, transparent);
  color: #4285f4;
}

[data-theme='dark'] .provider-item__icon--gemini {
