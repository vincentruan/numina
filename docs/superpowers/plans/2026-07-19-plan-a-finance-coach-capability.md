...
> **Follow-up (resolved).** While verifying, `tests/backend/test_ai_report.py` was found to have 4 pre-existing failures (NOT caused by Step 6): its `test_generate_report_*` cases patched the deleted `_ai_events_helper.AgentClient` and asserted `application/x-ndjson`, but the U4 SSE refactor moved `AgentClient` into `ai_report.py` and switched the route to `text/event-stream`. Fixed in a separate commit: replaced the two-phase NDJSON mock with an SSE passthrough mock, repointed the patches at `ai_report.AgentClient`, flipped the content-type assertions to `text/event-stream`, and updated the two task-status assertions from `completed`/`failed` to `running` (the SSE route is a pure passthrough — task lifecycle moved to the agent worker; backend `complete_task` has no report-path caller, verified by probing the real status). All 11 tests in `test_ai_report.py` now pass.
