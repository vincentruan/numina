...
**为什么**：P0 已交付增量价值（各触点闭环本地循环），P1 补齐体验一致性（spec §2）。**诚实定位（review #9）**：spec §1 的核心目标是「财务决策链」，但其实质进展主要靠 N1（finance hub + 概览首屏）；本批 14 项是支撑性增强，分三轨——(a) hygiene/死代码减少（D5/D6/A2）降维护噪音，无直接用户价值；(b) 信息密度与交互增强（D1/D3/D4/D7/A3/L3/L4）改善已有触点的可用性，间接支撑决策链；(c) 语义修正与体验一致性（L5/B4/F3/F7）。本批不单独推进决策链目标，而是为 N1 的决策链载体补齐周边体验。
...
- **[P2] 排除 N1 使批次对 spec §1 决策链目标贡献变薄** — **已处理（轻量诚实定位）**：Goal "为什么" 明确本批以 hygiene + 信息密度为主，决策链实质进展靠 N1，本批是支撑性增强（分 hygiene/信息密度/语义修正三轨），不夸大对决策链的进展。未拆独立章节（避免过度结构化）。
