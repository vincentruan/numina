...
- **[P1] U4 e2e — DeerMem 污染干扰 asset-report 三步流水线（U4 实现期发现，2026-07-18）** — U4 步骤 1/5/6/7 端到端验证（真实 backend+agent，Demo Family）发现：LLM 在 asset-report run 中直接复用 DeerMem 缓存的旧报告数据（"根据记忆中的信息..."）直答，**跳过 write_file/read_file/MCP 三步**。memory 污染源是 P0 试点（commit 1a27f076）生成的报告进了 family 的 DeerMem store。影响：(1) 真实使用时 memory 干扰三步执行，LLM 可能不调 write_file/read_file；(2) P0 pilot 90% 成功率是在 memory 干净时测的，memory 有数据后单遍成功率可能下降；(3) trigger force=true e2e 跑 120s 未完（662 messages 长输出，疑似 memory 助长的循环）。链路本身无 bug（422 已修，step2_json/落库/缓存均验证通过）。**决断：待定（Open）** — 候选方案：(a) asset-report skill 禁用 memory context（report 是固定流水线，不应受历史记忆污染）；(b) 清理试点遗留 memory 数据 + 复测 F1；(c) SKILL.md 显式指示"忽略 memory，重新调工具取数据"。需评估 DeerMem 禁用粒度（per-skill vs per-run）+ 复测 F1 确认影响。优先级 P1（影响真实使用 + F1 基线有效性）。
