...
- **[P2] U4 e2e — DeerMem 污染干扰 asset-report 三步流水线（U4 实现期发现，2026-07-18，已处理）** — U4 步骤 1/5/6/7 端到端验证（真实 backend+agent，Demo Family）发现：LLM 在 asset-report run 中直接复用 DeerMem 缓存的旧报告数据（"根据记忆中的信息..."）直答，**跳过 write_file/read_file/MCP 三步**。memory 污染源是 P0 试点（commit 1a27f076）生成的报告进了 family 的 DeerMem store。**关键事实修正**：P0 试点 20 次用同一 Demo Family + 同一默认 agent_name（lead-agent），前几次 run 写入 memory 后续读到，故 90% 成功率**已在 memory 累积环境下测得**——memory 污染**不 invalidate F1 基线**。e2e 那次 LLM 跳过三步是偶发漂移（试点 18/20 仍成功调了 write_file/read_file）。链路本身无 bug（422 已修，step2_json/落库/缓存均验证通过）。优先级从 P1 下调至 P2（不阻塞 U4，F1 基线有效）。**决断：Apply（2026-07-18，选项 1 — 复刻 DeerMem agent_name 分桶）** — DeerMem 原生按 `(agent_name, user_id)` 分桶（`deerflow/agents/memory/manager.py:48`），`DeerFlowClient.__init__` 接受 `agent_name` 参数。给 `create_family_adapter` / `get_family_adapter` / `DeerFlowAdapter.__init__` 加 `agent_name` 参数，worker `_run_asset_report_pipeline` 传 `agent_name="asset-report"`，cache key 纳入 agent_name（asset-report client 与 chat client 独立缓存）。asset-report 获独立 memory 桶，与 chat(lead-agent) 隔离——固定流水线每次取 fresh MCP 数据，不受 chat memory 污染。复刻原生机制，非自造 per-skill 禁用。候选 (b) 清理试点 memory + (c) SKILL.md 指示忽略 memory 不采纳（治标 / prompt 脆弱）。**2026-07-18 e2e 复验后发现 agent_name 隔离未生效**：worker 传 `agent_name="asset-report"` 给 `DeerFlowClient.__init__`（`self._agent_name`），但 e2e 仍见 `<memory>` 注入 20 处（含旧报告数据）。**根因**：memory 注入由 `DynamicContextMiddleware`（`dynamic_context_middleware.py:162` 调 `_get_memory_context(self._agent_name)`）负责，该中间件在 `build_middlewares(agent_name=...)`（`agent.py:280`）构造。`build_middlewares` 的 `agent_name` 在两条路径分叉：(1) `DeerFlowClient._build_agent`（client.py:292）用 `self._agent_name`；(2) `make_lead_agent`（agent.py:476/642）用运行时 `cfg.get("agent_name")`（来自 RunnableConfig.configurable）。numina sync `stream()` 走 `make_lead_agent` 路径，运行时 config 无 `agent_name` → `DynamicContextMiddleware(agent_name=None)` → `_get_memory_context(None)` 返回全局 lead-agent 桶（含旧数据）。**修复方向（待实施）**：numina `typed_stream_dispatch` 在运行时 config 注入 `agent_name`（`config={"configurable": {"agent_name": "asset-report"}}`），让 `make_lead_agent` 传到 `DynamicContextMiddleware`。涉及 adapter config 构造层 + deerflow 内部 `make_lead_agent` vs `_build_agent` 路径分叉，风险较大，不在 U4 主线修——记为 P2 Open，后续单独处理。F1 v2 100%（试点脚本路径）不阻塞——试点脚本可能不经 `make_lead_agent` 运行时路径或 memory 桶恰好空。
