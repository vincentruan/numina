...
> **Update note (2026-07-20):** The 6 raw-text streaming routers (`/alerts/stream`, `/allocation/stream`, `/liability/stream`, `/spending_leak/stream`, `/disposal/stream`, `/time_machine/stream`) and the `orchestrator.stream_dispatch()` method that Fix 1 patches were **deleted as dead code** in the two-AI-apps unified-dispatch refactor (U5/U7/U8) — the trigger-skill routers had been unreachable since commit `a97eb08c`. The `StreamChunk` type itself and `DeerFlowAdapter.stream_dispatch` survive on the adapter class. **Fix 1 is historical** (the patched code no longer exists). **Fix 2** (`family_id` validation in `backend_client.py:_validate_family_id`) **remains current.** **Fix 3 is historical** — `gateway.py` no longer has `_verify_token`; it now uses `Depends(verify_service_token)` with JWT-based auth from `packages/security/service_auth/agent_token_verify.py`, which is a stronger solution than `hmac.compare_digest`. **Fix 4 (DEERFLOW_GATEWAY_URL hostname allowlist) remains unapplied** — `server/apps/agent/app/config.py` still defines `DEERFLOW_GATEWAY_URL` with a default but has no `model_post_init` hostname validation. This is an open TODO.
