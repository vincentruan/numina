...
### Trade-off: 登录限流基于用户名而非 IP
→ **设计决策:** 登录速率限制使用用户名作为键，而非 IP 地址
→ **优势:**
  - 防止针对特定账户的暴力破解攻击
  - 多用户共享同一 IP（如企业网络）时不会相互影响
  - 攻击者无法通过更换 IP 绕过限流来攻击同一账户
→ **限制:**
  - 攻击者可以通过尝试不同用户名来绕过限流
  - 分布式攻击（多个 IP 攻击多个账户）不受此限制约束
→ **缓解措施:**
  - 全局 API 限流仍按 IP 标识，提供第二层防护
  - 安全日志记录所有登录失败事件，可检测大规模攻击模式
  - 未来可添加 IP 级别的登录限流作为补充（分层防御）

### Trade-off: 全局限流使用内存存储
→ **设计决策:** `RateLimitMiddleware` 使用类级别的内存字典存储限流状态
→ **限制:** 多 worker 进程部署时，每个 worker 维护独立的限流状态，可能导致实际限流阈值 = worker 数量 × 配置阈值
→ **缓解措施:**
  - 当前为单机单 worker 部署，不受影响
  - 已提供 `CacheBackend` 抽象层，未来可切换到 Redis 实现分布式限流
  - 如需多 worker 部署，建议实现 Redis 后端或使用 Nginx 限流