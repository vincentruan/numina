...
注意: 实现使用 datetime.now(UTC)（naive），测试时间戳也用 naive datetime 保持一致。
"""
...
def test_cleanup_mixed_sessions(packages_db):
    """混合场景: 仅过期未撤销的被标记。"""
    _make_session(packages_db, expires_at=datetime.now(UTC) - timedelta(hours=2),
                  is_revoked=False, jti="j1")
    _make_session(packages_db, expires_at=datetime.now(UTC) - timedelta(hours=3),
                  is_revoked=False, jti="j2")
    _make_session(packages_db, expires_at=datetime.now(UTC) + timedelta(days=1),
                  is_revoked=False, jti="j3")
    _make_session(packages_db, expires_at=datetime.now(UTC) - timedelta(hours=1),
...
def test_delete_mixed_sessions(packages_db):
    ...
    _make_session(packages_db, expires_at=datetime.now(UTC) - timedelta(days=10),
                  is_revoked=True, last_seen_at=datetime.now(UTC) - timedelta(days=9),
                  jti="d1")  # 删
    _make_session(packages_db, expires_at=datetime.now(UTC) - timedelta(days=20),
                  is_revoked=True, last_seen_at=datetime.now(UTC) - timedelta(days=15),
                  jti="d2")  # 删
    _make_session(packages_db, expires_at=datetime.now(UTC) - timedelta(days=10),
                  is_revoked=True, last_seen_at=datetime.now(UTC) - timedelta(days=1),
                  jti="k1")  # 留（最近见过）
    _make_session(packages_db, expires_at=datetime.now(UTC) + timedelta(days=1),
                  is_revoked=False, last_seen_at=datetime.now(UTC) - timedelta(days=30),
                  jti="k2")  # 留（活跃）
    deleted = delete_old_revoked_sessions(packages_db)
    assert deleted == 2
