...
def _patch_orchestrator(capability: str):
    ...
    from apps.agent.schemas.response import AgentResponse as AR
    resp = AR(**_make_response(capability))
    return patch(
        "apps.agent.services.orchestrator.Orchestrator.dispatch",
...
def client():
    ...
    with (
        patch("apps.agent.app.config.AgentSettings.validate_required", return_value=None),
        patch("apps.agent.services.orchestrator.settings.AGENT_INTERNAL_TOKEN", _TOKEN, create=True),
...
class TestChatEndpoint:
    ...
    def test_ask_passes_question_as_free_text(self, client):
        ...
        with patch("apps.agent.services.orchestrator.Orchestrator.dispatch", side_effect=_capture_dispatch):
            client.post(
                "/chat/ask",
    ...
    def test_ask_stream_uses_orchestrator_event_stream_with_question(self, client):
        ...
        with patch(
            "apps.agent.services.orchestrator.Orchestrator.stream_dispatch_events",
...
class TestAllocationEndpoint:
    ...
    def test_drift_passes_targets_as_free_text_json(self, client):
        ...
        with patch("apps.agent.services.orchestrator.Orchestrator.dispatch", side_effect=_capture):
            client.post(
                "/allocation/drift",
