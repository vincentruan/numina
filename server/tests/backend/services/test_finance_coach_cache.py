...
def test_latest_by_skill_isolates_finance_coach_from_report(db_session):
    ...
    report_latest = latest_by_skill(db_session, "1001", "asset-report")
    coach_latest = latest_by_skill(db_session, "1001", "finance-coach")
...
def test_invalidate_skill_deletes_only_that_skill(db_session):
    ...
    assert latest_by_skill(db_session, "1002", "finance-coach") is None
    assert latest_by_skill(db_session, "1002", "asset-report") is not None  # untouched
...
def test_invalidate_skill_scoped_to_one_family(db_session):
    ...
    assert latest_by_skill(db_session, "1003", "finance-coach") is None
    assert latest_by_skill(db_session, "1004", "finance-coach") is not None
