...
def update_template(
    db: Session, user: User, template_id: str, req: ChoreTemplateUpdate
) -> ChoreTemplate:
...
def toggle_template(
    db: Session, user: User, template_id: str, is_active: bool
) -> ChoreTemplate:
    t = get_template(db, user, template_id)
...
def get_or_create_instances(
    db: Session, child_user: User, date_str: str
) -> list[ChoreInstance]:
    ...
    for template in templates:
        ...
        owner_id = (
            ...
            if template.assignment_type == "pool"
            else child_user.id
        )
...
def _get_templates_for_child(db: Session, child_user: User) -> list[ChoreTemplate]:
    ...
    assigned_templates = (
        ...
        .join(
            chore_template_assignees,
            ChoreTemplate.id == chore_template_assignees.c.template_id,
        )
        .filter(
            ChoreTemplate.family_id == child_user.family_id,
...
async def approve_instance_async(
    db: Session, parent_user: User, instance_id: str
) -> ChoreInstance:
    ...
    if instance.status != "pending_approval":
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_CONTENT, "该实例不在待审批状态"
        )
    ...
    db.execute(
        text(
            "UPDATE chore_instances SET streak_count=:streak, streak_bonus=:bonus WHERE id=:id"
        ),
    ...
    if not family or not child:
        narrative, emoji = (
            f"你完成了{instance.chore_name}！获得 {actual_amount} 颗星",
            "⭐",
        )
    else:
    ...
    try:
        ...
        logger.warning(
            "Failed to increment total_approved_count for child %s — ignoring",
            coin_recipient_id,
        )
    try:
        db.add(tx)
    ...
    try:
        ...
        template = db.query(ChoreTemplate).filter_by(id=instance.template_id).first()
        if (
            config
            and config.education_reward_enabled
            and template
            and template.real_reward_enabled
        ):
    ...
    milestone = check_and_record_milestones(
        ...
        {
            "instance": instance,
            "total_approved_count": child.total_approved_count if child else 0,
        },
    )
    instance._milestone_triggered = milestone  # type: ignore[misc]
    ...
    try:
        ...
        logger.warning(
            "Challenge progress check failed for child %s — ignoring", coin_recipient_id
        )
...
def assign_instance(
    db: Session, parent_user: User, instance_id: str, target_child_id: int
) -> ChoreInstance:
...
def claim_instance(db: Session, child_user: User, instance_id: str) -> ChoreInstance:
    ...
    if (
        instance.assigned_by_user_id is not None
        and instance.child_user_id != child_user.id
    ):
        raise HTTPException(status.HTTP_404_NOT_FOUND, "家务实例不存在")
...
def reject_instance(
    db: Session, parent_user: User, instance_id: str, return_to_redo: bool = False
) -> ChoreInstance:
    instance = _get_family_instance(db, parent_user, instance_id)
    if instance.status != "pending_approval":
        raise HTTPException(
...
def list_pending_approvals(db: Session, parent_user: User) -> list[ChoreInstance]:
    ...
    economy = (
        db.query(ChildEconomyConfig).filter_by(family_id=parent_user.family_id).first()
    )
    ...
    submitter_ids = {
        ...
        else (i.child_user_id if i.child_user_id != i.family_id else None)
        for i in result
    } - {None}
    child_map: dict[int, User] = {}
    if submitter_ids:
    ...
    for instance in result:
        ...
        lookup_id = instance.submitted_by_user_id or (
            instance.child_user_id
            if instance.child_user_id != instance.family_id
            else None
        )
        child = child_map.get(lookup_id) if lookup_id else None  # type: ignore[arg-type]
        instance._child_display_name = child.display_name if child else None  # type: ignore[misc]
        instance._child_avatar_color = child.avatar_color if child else None  # type: ignore[misc]
        instance._child_user_id = lookup_id  # type: ignore[misc]
...
def _auto_approve(db: Session, instance: ChoreInstance, family: Family) -> None:
    ...
    if multiplier >= 2.0:
        ...
        narrative = (
            f"你完成了{instance.chore_name}！连续打卡加成，获得 {actual_amount} 颗星 🔥"
        )
    else:
    ...
    check_and_record_milestones(
        db, coin_recipient_id, family.id, {"instance": instance}
    )
...
def _compute_streak(db: Session, instance: ChoreInstance) -> int:
    ...
    template = (
        db.query(ChoreTemplate).filter(ChoreTemplate.id == instance.template_id).first()
    )
    if not template:
        return 1
...
def _is_consecutive(prev_bucket: str, curr_bucket: str, frequency: str) -> bool:
    ...
    if frequency == "daily":
        ...
        def parse_week(s: str) -> date:
            year, week = s.split("-W")
            d: date = datetime.strptime(
                f"{year}-W{int(week):02d}-1", "%G-W%V-%u"
            ).date()
            return d
...
def _validate_assignees(db: Session, family_id: int, assignee_ids: list[int]) -> None:
    for uid in assignee_ids:
        user = (
            db.query(User).filter(User.id == uid, User.family_id == family_id).first()
        )
        if not user:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_CONTENT, f"用户 {uid} 不属于该家庭"
            )
        if user.role != "child":
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_CONTENT, f"用户 {uid} 不是孩子角色"
            )


def _get_child_instance(
    db: Session, child_user: User, instance_id: str
) -> ChoreInstance:
