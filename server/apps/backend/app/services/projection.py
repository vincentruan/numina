...
def calculate_projection(
    ...
    for y in range(projection_years + 1):
        ...
        for ap in asset_projections:
            current_value = ap["current_value"] or 0
            annual_return = ap["annual_return"] or 0
            depreciation = ap["depreciation"] or 0
            if ap["asset_type"] == "financial":
                val = current_value * ((1 + annual_return) ** y)
            else:
                val = current_value * ((1 - depreciation) ** y)
            total_assets += max(val, 0)

        total_liabilities = 0.0
        for lp in liability_projections:
            remaining = (lp["remaining"] or 0) - (lp["monthly_payment"] or 0) * 12 * y
