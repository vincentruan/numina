...
def upgrade() -> None:
    ...
    # Set agent_type='builtin' where is_builtin=true (these are predefined agents)
    op.execute(
        "UPDATE ai_agents SET agent_type = 'builtin' WHERE is_builtin = true"
    )

    # Set agent_type='custom' where is_builtin=false (these are user-created agents)
    # Handles both family_id>0 (normal case) and family_id=0 (edge case/data error)
    op.execute(
    ...
    # 4. Drop the old index on is_builtin
    op.drop_index('ix_ai_agents_builtin', table_name='ai_agents')

    # 5. Create new index on agent_type
    op.create_index(
        'ix_ai_agents_type',
        'ai_agents',
        ['agent_type'],
    ...
    # 6. Insert system agents (ai-assistant, time-machine)
