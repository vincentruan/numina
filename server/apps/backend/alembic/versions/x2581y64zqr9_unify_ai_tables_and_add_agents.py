...
def upgrade() -> None:
    ...
    # (guarded: on fresh DB these may already have the unified name if a prior
    #  branch created them; on legacy DB they have the old name.)
    def _rename_if_exists(old: str, new: str) -> None:
        if bind.dialect.has_table(bind, old):
            op.rename_table(old, new)
        elif not bind.dialect.has_table(bind, new):
            # neither exists — shouldn't happen, but create nothing
            pass

    _rename_if_exists("ai_provider_configs", "ai_providers")
    _rename_if_exists("family_mcp_servers", "ai_mcp_servers")
    _rename_if_exists("skill_registry", "ai_skills")

    # 2. Create ai_agents table (guard: skip if already exists)
    if not bind.dialect.has_table(bind, 'ai_agents'):
        # The agent_name regex CheckConstraint uses Postgres `~` operator which
        # SQLite does not support; include it only on Postgres. App-layer
        # validation enforces the format regardless.
        constraints = [
    ...
    # Postgres-only: the seed uses `::jsonb` casts (SQLite has no jsonb). Test
    # DBs (create_all) and app-level seeding cover fresh SQLite; skip there.
    if not is_sqlite:
        op.execute("""
