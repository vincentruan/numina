...
class PolicyGuard:
    ...
    def check(self, policy: CapabilityPolicy, skill_id: str) -> PolicyDecision:
        if not policy.ai_enabled:
            return PolicyDecision(allowed=False, reason="AI功能未启用")

        if policy.allowed_capabilities and skill_id not in policy.allowed_capabilities:
            return PolicyDecision(allowed=False, reason="该功能不可用")

        if skill_id in policy.admin_only_capabilities and policy.member_role != "admin":
