"""TodoMiddleware — numina's plan-mode todo tracking middleware.

Subclasses langchain's ``TodoListMiddleware`` (which provides the
``write_todos`` tool + default system_prompt / tool_description) and adds the
two DeerFlow behaviours:

1. **Context-loss reminder (``before_model``)** — when the message history is
   truncated (e.g. by SummarizationMiddleware), the original ``write_todos``
   tool call and its ToolMessage can scroll out of the active context window.
   This hook detects that gap and injects a hidden reminder so the model still
   knows about the outstanding todo list.

2. **Premature-exit prevention (``after_model``)** — when the model produces a
   final response (no tool calls) but todos are not yet complete, the middleware
   queues a completion reminder for the next model request and jumps back to the
   model node to force continued engagement. A retry cap
   (``_MAX_COMPLETION_REMINDERS``) prevents infinite loops.

R2 — sync hook requirement
--------------------------
Numina's adapter runs the agent via the SYNC ``DeerFlowClient.stream()`` (in a
thread executor), so LangGraph dispatches the SYNC ``before_model`` /
``after_model`` / ``wrap_model_call`` hooks. An async-only middleware would
compile + mount successfully but silently do nothing on this path (see
``run_extras.py`` title-middleware note for the same trap). This subclass
therefore implements BOTH the sync and async hooks; the async variants delegate
to the sync ones.

R3 — singleton
--------------
``family_adapter_cache.py:726`` keys the LRU cache on
``tuple(id(m) for m in middlewares)``. A fresh ``TodoMiddleware()`` per call
would bust the cache every call → agent rebuild every call → tool-loading
races. Use ``get_todo_middleware()`` (module-level singleton) so ``id()`` is
...
state access uses duck-typed ``state.get(...)`` (the compiled DeerFlow graph's
``ThreadState`` is a TypedDict that supports ``.get``), so no local
``ThreadState`` import is needed.
"""
...
class TodoMiddleware(TodoListMiddleware):
    ...
    When the original ``write_todos`` tool call has been truncated from the
    message history (e.g. after summarization), the model loses awareness of
    the current todo list. This middleware detects that gap in
    ``before_model`` / ``abefore_model`` and injects a reminder message so the
    model can continue tracking progress.

    Additionally ``after_model`` prevents premature agent exit while todo items
    remain incomplete (see class docstring of the module).
    """
    ...
    def before_model(
        ...
        todos: list[Todo] = state.get("todos") or []  # type: ignore[union-attr]
        if not todos:
            return None

        messages = state.get("messages") or []  # type: ignore[union-attr]
    ...
    def after_model(
        ...
        there are still incomplete todo items. It queues a reminder and jumps
        back to the model node so the agent continues working through the todo
        list.
...
# ── Module-level singleton (R3 — stable id() for family_adapter_cache LRU key) ──
# ``family_adapter_cache.get_family_adapter`` keys the LRU cache on
# ``tuple(id(m) for m in middlewares)``. A fresh ``TodoMiddleware()`` per call
# would bust the cache every call → agent rebuild every call. This singleton
# keeps ``id()`` stable across calls within a process.
_TODO_MIDDLEWARE_SINGLETON: TodoMiddleware | None = None
_SINGLETON_LOCK = threading.Lock()


def get_todo_middleware() -> TodoMiddleware:
    """Return the process-wide TodoMiddleware singleton.

    Created lazily on first call (so the langchain import only happens when
    plan_mode is actually used). Thread-safe; the instance itself is safe for
    concurrent use (all mutable state is guarded by ``self._lock`` and keyed by
    ``(thread_id, run_id)``).
    """
    global _TODO_MIDDLEWARE_SINGLETON
    if _TODO_MIDDLEWARE_SINGLETON is None:
        with _SINGLETON_LOCK:
            if _TODO_MIDDLEWARE_SINGLETON is None:
                _TODO_MIDDLEWARE_SINGLETON = TodoMiddleware()
    return _TODO_MIDDLEWARE_SINGLETON
