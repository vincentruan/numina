...
class TestResolveToolMetadata:
    ...
    def test_exact_match_returns_registry_entry(self) -> None:
        ...
        tool_type, display_name, icon, display_key = resolve_tool_metadata("execute_code")
        assert tool_type == "execution"
        assert display_name == "执行分析代码"
        assert icon == "⚙️"
        assert display_key == "toolName.executeCode"
    ...
    def test_exact_match_mcp_tool(self) -> None:
        ...
        assert icon == "💰"
        assert display_key == "toolName.getAssetsData"

    def test_suffix_match_whitelisted_bash(self) -> None:
        """Namespaced variants of whitelisted DeerFlow tools should match."""
        tool_type, display_name, icon, display_key = resolve_tool_metadata("some_namespace_bash")
        assert tool_type == "execution"
        assert display_name == "执行命令"
        assert icon == "⚙️"
        assert display_key == "toolName.bash"

    def test_suffix_match_whitelisted_write(self) -> None:
        """Suffix match for write should work."""
        tool_type, display_name, icon, display_key = resolve_tool_metadata("custom_write")
        assert tool_type == "execution"
        assert display_name == "写入结果"
        assert icon == "📝"
        assert display_key == "toolName.write"
    ...
    def test_suffix_match_blocked_for_non_whitelisted(self) -> None:
        ...
        tool_type, display_name, icon, display_key = resolve_tool_metadata("audit_log_get_assets")
    ...
    def test_suffix_match_blocked_for_non_whitelisted_suffix(self) -> None:
        ...
        tool_type, display_name, icon, display_key = resolve_tool_metadata("audit_log_compose_summary")
        assert tool_type == "unknown"
    ...
    def test_unknown_tool_returns_fallback(self) -> None:
        ...
        tool_type, display_name, icon, display_key = resolve_tool_metadata("unknown_tool_xyz")
        assert tool_type == "unknown"
    ...
    def test_no_underscore_skips_suffix_match(self) -> None:
        ...
        tool_type, display_name, icon, display_key = resolve_tool_metadata("unknownsingleword")
        assert tool_type == "unknown"
