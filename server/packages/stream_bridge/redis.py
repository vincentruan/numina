...
Self-contained Numina implementation — no DeerFlow dependency.
Includes :class:`NuminaRedisStreamBridge` with tenant isolation
(``family_id`` in Redis keys).
"""
...
class RedisStreamBridge(StreamBridge):
    ...
    def __init__(
        ...
        *,
        ...
        self._stream_ttl_seconds: int | None
...
class NuminaRedisStreamBridge(RedisStreamBridge):
    """Numina-flavored Redis Streams bridge with tenant isolation.

    Inherits all Redis Streams logic (XADD/XREAD, heartbeat, StreamGap
    detection, 24h TTL) and customizes the key format to include family_id
    for strict tenant isolation at the Redis level.
    ...
    def _stream_key(self, run_id: str) -> str:
        ...
        The run_id is expected to be in format ``"family_id:run_id"`` or
        just ``"run_id"``.  If family_id is not included, uses ``"0"``
        as default.
