...
| **G1** adult-stable | [`g1-adult-stable/`](./g1-adult-stable/) | area2, area3, area6, area7, area8, area11, **area12**, **area14** | `$SID` (adult) | reads global; per-entity writes only | **G3** |
...
```
        ┌── G1 (adult-stable: area2→area14→area3→area6→area8→area12→area7→area11) ──┐
