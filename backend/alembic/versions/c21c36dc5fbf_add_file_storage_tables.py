...
def upgrade() -> None:
    ...
    if 'storage_backends' not in existing_tables:
        op.create_table(
            'storage_backends',
    ...
    if 'cached_files' not in existing_tables:
        op.create_table(
            'cached_files',
    ...
    if 'file_remote_locations' not in existing_tables:
        op.create_table(
    ...
    if 'sync_events' not in existing_tables:
        op.create_table(
            'sync_events',
            sa.Column('id', sa.String(36), primary_key=True),
            sa.Column('file_id', sa.String(36), sa.ForeignKey('cached_files.id'), nullable=False),
