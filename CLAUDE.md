...
- **Past solutions** — `docs/solutions/` contains documented fixes for recurring problems. Check before debugging known issue categories. Subdirectories: `architecture-patterns/` (e.g. three-state circuit breaker with cascade retry), `best-practices/` (e.g. Redis fail-fast strategy, cache key granularity, Pydantic validation), `workflow-issues/` (e.g. backend module extraction workflow), `integration-issues/` (e.g. DeerFlow silent fallback), `test-failures/`, `ui-bugs/`, `developer-experience/`.
